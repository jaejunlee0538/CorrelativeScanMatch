...
for sc=1:size(laserEndPntsMapFrame, 2)
    ...
    [X, Y] = bresenham(robPoseMapFrame, laserEndPntsMapFrame(1:2, sc));
...
for i = 1:size(freeCells, 2)
    mapUpdate(freeCells(1,i), freeCells(2,i)) = l_free;
end
...
for i = 1:size(laserEndPntsMapFrame, 2)
    mapUpdate(laserEndPntsMapFrame(1,i), laserEndPntsMapFrame(2,i)) = l_occ;
end
end
